...
fn main() {
    let content =
        fs::read_to_string("rotten-script-wasm/node-project/sample/sample2/sample2.rots").unwrap();
    ...
    parser.parse().unwrap();
    let ast = parser.ast;
    let tree = analyze(vec![("sample2/sample2.rots".to_string(), &ast)]);
    let result = tree.call_builder(true, &logger);
